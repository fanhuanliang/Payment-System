const fontSizeBase = "1";

const variable = {
  // font
  font_family_monospace: `SFMono-Regular, Menlo, Monaco, Consolas,
  'Liberation Mono', 'Courier New'`,
  font_family_base: `-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto,
  'Helvetica Neue', Arial, 'Noto Sans', sans-serif, 'Apple Color Emoji',
  'Segoe UI Emoji', 'Segoe UI Symbol', 'Noto Color Emoji'`,
  font_size_base: "1rem",
  font_weight_lighter: "lighter",
  font_weight_light: "300",
  font_weight_normal: "400",
  font_weight_bold: "700",
  font_weight_bolder: "bolder",
  font_weight_base: "400",

  // theme
  white: "#ffffff",
  gray_100: "#f8f9fa",
  gray_200: "#e9ecef",
  gray_300: "#dee2e6",
  gray_400: "#ced4da",
  gray_500: "#adb5bd",
  gray_600: "#6c757d",
  gray_700: "#495057",
  gray_800: "#343a40",
  gray_900: "#212529",
  black: "#000",
  blue: "rgb(224, 230, 239)",
  indigo: "#6610f2",
  purple: "#6f42c1",
  pink: "#d63384",
  red: "#dc3545",
  orange: "#fd7e14",
  yellow: "#fadb14",
  dark_yellow: "#ffb533",
  green: "#b8e356",
  teal: "rgb(152,230,240)",
  cyan: "#17a2b8",
  dark_blue: "rgb(0,108,182)",
  border_gray: "rgb(203, 207, 209)",
  light_blue: "rgb(163, 205, 217)",
  dark_green: "#44c767",

  // line
  line_height_base: "1.5",
  line_height_lg: "2",
  line_height_sm: "1.25",

  // heading
  h1_font_size: `${fontSizeBase * 2.5}rem`,
  h2_font_size: `${fontSizeBase * 2}rem`,
  h3_font_size: `${fontSizeBase * 1.75}rem`,
  h4_font_size: `${fontSizeBase * 1.5}rem`,
  h5_font_size: `${fontSizeBase * 1.25}rem`,
  h6_font_size: `${fontSizeBase}rem`,
};

export default variable;
